...
describe("useSemanticModelQuery", () => {
    ...
    it("passes bypassCache: true through to the query when refetch is called with an override", async () => {
        const successResult = {
            status: "success" as const,
            table: { columns: [], rows: [] },
            fromCache: false,
        ...
        // Hook itself defaults to bypassCache: false (i.e. cache-first) — a manual
        // refresh action should still be able to force a true bypass regardless.
        const { result } = renderHook(() =>
            useSemanticModelQuery({ connection: "model", query: "EVALUATE ROW()", bypassCache: false }),
        );

        await waitFor(() => expect(result.current.isLoading).toBe(false));
        expect(mockQuery).toHaveBeenLastCalledWith("EVALUATE ROW()", { bypassCache: false });

        await act(async () => {
            await result.current.refetch({ bypassCache: true });
        });
