...
interface UseSemanticModelQueryResult {
    ...
    refetch: (overrideOptions?: { bypassCache?: boolean }) => Promise<void>;
}
...
export function useSemanticModelQuery(
    ...
    const execute = useCallback(async (overrideOptions?: { bypassCache?: boolean }) => {
        ...
        try {
            const result = await getFabricClient()
                ...
                .query(query, { bypassCache: overrideOptions?.bypassCache ?? bypassCache });
